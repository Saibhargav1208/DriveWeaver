#!/bin/bash
#SBATCH --job-name=abl1_pscore
#SBATCH --output=/data1/work/j0987341/aadya/research/DriveWeaver/logs/ablation1_planner_scoreloss-%j.out
#SBATCH --error=/data1/work/j0987341/aadya/research/DriveWeaver/logs/ablation1_planner_scoreloss-%j.err
...
WORKDIR=/work
SCRIPT=training/train_planner_a.py
CONFIG=default

CACHE_DIR=/work/data/future_slots/cjepa
PLANNER_DIR=/work/checkpoints/planner_a
...
log_section "Train Planner A with proposal score loss"
docker exec \
  -e NVIDIA_VISIBLE_DEVICES="${VISIBLE_UUIDS}" \
  -e CUDA_VISIBLE_DEVICES="${VISIBLE_UUIDS}" \
  "${CONTAINER}" \
...
log_section "Score-loss Planner A training complete"
