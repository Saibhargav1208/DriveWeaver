#!/bin/bash
#SBATCH --job-name=abl2_pcache
#SBATCH --output=/data1/work/j0987341/aadya/research/DriveWeaver/logs/ablation2_planner_cached-%j.out
#SBATCH --error=/data1/work/j0987341/aadya/research/DriveWeaver/logs/ablation2_planner_cached-%j.err
...
if [[ "${RESET_PLANNER_DIR}" == "true" ]]; then
  ...
  BACKUP_DIR="${PLANNER_DIR}_before_cached_${SLURM_JOB_ID}_$(date +%Y%m%d-%H%M%S)"
...
log_section "Train Planner B from cached future slots"
docker exec \
  -e NVIDIA_VISIBLE_DEVICES="${VISIBLE_UUIDS}" \
  -e CUDA_VISIBLE_DEVICES="${VISIBLE_UUIDS}" \
  "${CONTAINER}" \
...
log_section "Cached Planner B training complete"
