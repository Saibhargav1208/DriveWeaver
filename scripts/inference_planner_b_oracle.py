...
def main():
    ...
    parser.add_argument('--future_slots_cache_dir', type=str, default=None,
                        help='Optional precomputed future-slot cache dir with train.pt/val.pt; skips world-model inference')
    ...
    # Load Planner
    print(f"\n[2/2] Loading Planner B..." if not use_cached_future_slots else f"\n[1/1] Loading Planner B...")
    ...
    if not use_cached_future_slots:
        ...
        print(f"\n✓ Planner loaded with precomputed future slots!")

    # Load data. Cached future slots are the fast path for the newly trained Planner B.
    if use_cached_future_slots:
        cache_path = Path(args.future_slots_cache_dir) / f'{args.split}.pt'
        print(f"\nLoading cached {args.split} future slots from {cache_path}...")
