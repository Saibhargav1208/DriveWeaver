"""
Visualize Planner B Predictions with ORACLE Selection on nuScenes Images
...
**USES ORACLE SELECTION (minADE)** instead of scorer.
...
def main():
    ...
    for vis_idx, sample_idx in enumerate(tqdm(sample_indices, desc="Processing")):
        ...
        # Visualize
        title = f"Planner B (Oracle + VLM) | Sample {vis_idx+1} | {scene_token} @ frame {start_idx}"
        save_path = output_dir / f"planner_b_oracle_sample_{vis_idx:04d}_idx{sample_idx}.png"
    ...
    with open(summary_path, 'w') as f:
        f.write(f"Planner B (Ablation 2) - Validation Metrics (ORACLE SELECTION)\n{'='*60}\n")
        for k, v in metrics_summary.items():
        ...
        f.write(f"VLM Guidance: Enabled (Qwen3-VL layers 6,12,18,24)\n")
